...
func (state State) MakeBlock(
	...
	// F5 item 3: execution valset hash before Populate (block.ValidatorsHash is still empty).
	valSetHash := state.Validators.Hash()
	if !(os.Getenv("SECRET_NODE_MODE") == "replay") {
		...
		if err := tmenclave.ValidateRandom(random, proof, state.AppHash, valSetHash, uint64(block.Height)); err != nil {
