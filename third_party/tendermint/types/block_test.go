		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.header != nil && tc.expectHash != nil {
				...
				for i := 0; i < s.NumField(); i++ {
					f := s.Field(i)
					if s.Type().Field(i).Name == "EncryptedRandom" {
						continue
					}
...
func TestHeaderHashAndValidateBasicIgnoreEncryptedRandom(t *testing.T) {
	h := Header{
		Version:            cmtversion.Consensus{Block: version.BlockProtocol, App: 2},
		ChainID:            "chainId",
		Height:             3,
		Time:               time.Date(2019, 10, 13, 16, 14, 44, 0, time.UTC),
		LastBlockID:        makeBlockID(make([]byte, tmhash.Size), 6, make([]byte, tmhash.Size)),
	...
	hashNil := h.Hash()
	h.EncryptedRandom = &EnclaveRandom{Random: cmtrand.Bytes(48), Proof: cmtrand.Bytes(32)}
	require.NoError(t, h.ValidateBasic())
	require.Equal(t, hashNil, h.Hash())
